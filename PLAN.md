...
- Recent accepted reconstruction/motion fixes include luma Intra_8x8 filtered DC references, FFmpeg chroma DC quadrant/edge predictors, FFmpeg `pred_intra_mode` unavailable-neighbour handling, separate I4x4-derived right/bottom mode caches for CABAC I8x8 neighbour prediction, aligned-buffer reconstruction for partial edge macroblocks, FFmpeg-scale 8×8 dequant before IDCT, available top-right references from already reconstructed rows, FFmpeg-exact I8x8 horizontal-down and vertical-right predictors, partial-edge chroma reconstruction, B-slice MB-type default-branch parity, shaped B_8x8/sub-partition MV cache write-back, separate CABAC B L0/L1 MVD context caches, direct B_8x8 write-back into both list caches, shape-derived 16×8/8×16 directional MVP routing for all two-part B MB types, reference-frame-only DPB list filtering, and saved per-frame list0 4×4 motion/ref metadata for future Direct-mode colocated checks. Per-block I4x4/I8x8 luma and chroma reconstruction matches FFmpeg with loop filter disabled; B-frame quality now depends mainly on proper FFmpeg-style Direct-mode shape/colocated derivation.

Still gated:

- B-slice Direct-mode reconstruction is still a bounded fallback rather than FFmpeg-equivalent colocated temporal/spatial derivation. Full `B_Direct_16x16` currently uses a single spatial fallback MV, but FFmpeg can internally derive 8×8/direct sub-shapes from the colocated future MB even when the syntax is not `B_8x8`. Direct sub-MB modes now seed/write back spatial fallback motion for neighbour parity, saved reference frames carry list0 4×4 motion/ref caches, and the B_8x8 zero helper follows FFmpeg's `x8*3/y8*3` representative rule; remaining work is to preserve/derive colocated MB shape/type metadata and apply the same Direct-mode shape decisions for full direct MBs.
- Multi-frame CABAC diagnostics are now usable for post-motion B parity via `scripts/b_bidi_trace.sh`/`scripts/compare_bidi_trace.py`; the reference B frame is clean through `MB_LIMIT=200` (`compared=71 diffs=0`) and first diverges around `mb=0336` with a broader `MB_LIMIT=1000` window. `scripts/b_direct_trace.sh` also emits `FFCOLZERO/FFCOLZERO8` and `GOCOLZERO` rows, and `scripts/compare_colzero_trace.py` can focus colocated-zero gaps by `--mb`/`--part`; the current first direct-mode proof point is `mb=0268 part=3`, where FFmpeg zeroes one sub-MV after deriving colocated 8×8 handling for a non-syntactic full-direct row, while Go does not yet enter that path.
...
- Decoder and `trace264` now share the same QP wraparound semantics and 4×4 MV/ref-cache source-of-truth model; B-intra QP deltas are read from parsed intra payloads in both decode/trace flows, stale macroblock-level trace MV context was removed, B_8x8 direct sub-MBs are written back to both list caches, two-part B MVP routing is derived from partition shape, and MV cache read/fill helpers reject bad strides, short slices, and negative origins.
