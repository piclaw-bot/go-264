...
- Multi-frame CABAC diagnostics are now usable for post-motion B parity via `scripts/b_bidi_trace.sh`/`scripts/compare_bidi_trace.py`, plus `scripts/compare_bpart_mvd.py` for FFmpeg-vs-Go partition MVD/MVP/final-MV triage. After the B cache layer, direct write-back diagnostics, P16x8 top-before-bottom MVP fix, cropped-edge trace fixes, and Direct-flag normalization, the old `mb=0371` mismatch is classified as trace-shape noise. The current real broad-window proof point is bottom-row CABAC MVD context drift at `mb=0903`: FFmpeg decodes `mvd={0,-1}` while Go decodes `mvd={1,-1}` from `amvd={1,2}`, with matching `mvp={-1,-2}`. `scripts/b_direct_trace.sh` also emits `FFCOLZERO/FFCOLZERO8`, `GOCOLZERO`, `GOMOTSAVE`, `GOMOTWRITE`, and a `gowrite.diff` artifact so write-back regressions are caught separately from real FFmpeg-vs-Go Direct-mode derivation mismatches.
...
| `bbb_annexb.h264` 300-frame average PSNR | Y=8.99 U=19.25 V=29.01 dB |
