...
- Recent accepted reconstruction/motion fixes include luma Intra_8x8 filtered DC references, FFmpeg chroma DC quadrant/edge predictors, FFmpeg `pred_intra_mode` unavailable-neighbour handling, separate I4x4-derived right/bottom mode caches for CABAC I8x8 neighbour prediction, aligned-buffer reconstruction for partial edge macroblocks, FFmpeg-scale 8×8 dequant before IDCT, available top-right references from already reconstructed rows, FFmpeg-exact I8x8 horizontal-down and vertical-right predictors, partial-edge chroma reconstruction, B-slice MB-type default-branch parity, shaped B_8x8/sub-partition MV cache write-back, separate CABAC B L0/L1 MVD context caches, direct B_8x8 write-back into both list caches, shape-derived 16×8/8×16 directional MVP routing for all two-part B MB types, and reference-frame-only DPB list filtering. Per-block I4x4/I8x8 luma and chroma reconstruction matches FFmpeg with loop filter disabled; B-frame quality now depends mainly on proper FFmpeg-style Direct-mode derivation.

Still gated:

- B-slice Direct-mode reconstruction is still a bounded fallback rather than FFmpeg-equivalent colocated temporal/spatial derivation. Full `B_Direct_16x16` currently uses list0 fallback, which improves later B frames but is not the final algorithm. Direct sub-MB modes now seed/write back spatial fallback motion for neighbour parity, but still need colocated MV/ref derivation before broader Main/High quality can be called complete.
- Multi-frame CABAC diagnostics are now usable for post-motion B parity via `scripts/b_bidi_trace.sh`/`scripts/compare_bidi_trace.py`; the reference B frame is clean through `MB_LIMIT=200` (`compared=71 diffs=0`) and first diverges around `mb=0277` with a broader `MB_LIMIT=1000` window. The next diagnostic gap is still full FFmpeg-equivalent direct temporal/spatial derivation, not CABAC syntax state.
