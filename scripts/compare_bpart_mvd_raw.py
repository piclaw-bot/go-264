...
GO_RE = re.compile(
    r'GOBPART_MVD_RAW mb=(?P<mb>\d+)(?: poc=(?P<poc>-?\d+))? part=(?P<part>\d+) list=(?P<list>\d+) '
...
def load_go(path: str, occurrence: int, poc_filter: int | None) -> dict[tuple[int, int, int], dict[str, tuple[int, int]]]:
    out = {}
    seen: defaultdict[tuple[int, int, int], int] = defaultdict(int)
    for line in open(path, errors='replace'):
        m = GO_RE.search(line)
...
def main() -> None:
    ...
    ap.add_argument('--go-poc', type=int, help='optional Go picture POC filter when rows include poc=')
    ...
    go = load_go(args.gobpart_mvd, args.go_occurrence, args.go_poc)
    compared = diffs = 0
    if not ff:
