...
FF_RE = re.compile(
    r'FF_BPART_MVD mb=(?P<mb>\d+) part=(?P<part>\d+) list=(?P<list>\d+) '
...
def load_ff(path: str, occurrence: int) -> dict[tuple[int, int, int], dict[str, tuple[int, int]]]:
    out = {}
...
def main() -> None:
    ...
    args = ap.parse_args()
    ff = load_ff(args.ffbpart_mvd, args.ff_occurrence)
