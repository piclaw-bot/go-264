...
For reconstruction triage, `scripts/recon_i8x8_compare.py` compares Go `GORECON part=i8x8` and FFmpeg `FFRECON part=i8x8` logs by `(frame, mb, b8, occurrence)`. It supports focused filters (`--frame`, `--mb`, `--b8`, `--occurrence`), prediction/residual splits (`--max-pred-delta`, `--min-pred-delta`, `--sort out|pred|res`), mode filters (`--mode-mismatch`), and summaries (`--summary-by-mode`, `--summary-spatial`). `scripts/i8x8_mode_compare.py` compares Go `trace264` I8x8 predicted/decoded modes and edge-cache inputs against local FFmpeg `FFMODE part=i8x8` rows; after the accepted cache and partial-edge fixes, it reports no decoded-mode mismatches for the compared `bbb` first-frame I8x8 rows. `scripts/i4x4_mode_compare.py` performs the matching Go/FFmpeg comparison for I4x4 raw/final/writeback rows. B-direct work uses `scripts/b_direct_trace.sh` to patch local FFmpeg `h264_direct.c` and emit deterministic `FFDIRECT` rows, `GO264_DIRECT_TRACE=1` to emit matching `GODIRECT` rows from Go, and `scripts/compare_direct_trace.py` to compare `(ref0, ref1, mv0, mv1)`, direct sub-MB types, and per-sub-block direct MVs. The same script now also emits `FFCOLZERO/FFCOLZERO8` and `GOCOLZERO` rows for colocated Direct-mode zero checks; `scripts/compare_colzero_trace.py` can focus those rows with `--mb`/`--part` and exposes where FFmpeg runs colocated-zero logic but Go does not. Current evidence for the first direct mismatch is `mb=0268 part=3`: FFmpeg derives 8×8 colocated-zero handling for a non-syntactic `B_Direct_16x16` row (`is_b8x8=0`, colocated type `12352`, `sub_type=12552`, `mb_type=143680`) and zeroes `submv3`, while Go still treats the full direct MB as one spatial fallback MV. Post-motion B-slice work uses `scripts/b_bidi_trace.sh` to emit FFmpeg `FFBIDI` rows after ref/MVD/MVP write-back and Go `GOBIDI` rows after reconstruction-time motion resolution, then `scripts/compare_bidi_trace.py` compares only used-list representative cache entries so stale/unused FFmpeg cache cells do not drive fixes. The current reference B-frame post-motion window is clean through `MB_LIMIT=200` (`compared=71 diffs=0`); after B-list cache and direct diagnostics work, the first broader-window mismatch is around `mb=0336`. FFmpeg `frame_num` repeats across B pictures, so select repeated groups with `--ff-occurrence` when comparing beyond the first occurrence.
...
- MV/ref caches are the single source of motion-prediction context in the decoder and trace tooling. Cache reads/fills and CABAC MV/ref context helpers reject malformed strides, short slices, and negative origins instead of panicking in direct helper/tool use. CABAC B-slice motion now keeps separate L0/L1 MVD context caches, writes B_8x8 direct sub-MB motion back into both list caches, and routes all two-part B MB types through shape-derived 16×8/8×16 directional MVP helpers. CABAC MVD context cache write-back mirrors FFmpeg by keeping the full signed MVD for reconstruction while storing `min(abs(mvd),70)` for future context selection.
