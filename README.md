...
Current first-divergence status: `testsrc_cabac_p.h264` and `bbb_annexb.h264` first-frame MB syntax summaries now report `NO_DIVERGENCE in compared fields`. The active mismatch class has moved to reconstruction parity. Recent source-grounded fixes promoted FFmpeg-style CABAC intra edge contexts, High-profile intra `transform_size_8x8_flag` consumption, luma Intra_8x8 filtered DC references, the top-edge Intra_8x8 `LEFT_DC_PRED` reconstruction case, FFmpeg chroma DC quadrant/edge prediction, FFmpeg `pred_intra_mode` unavailable-neighbour handling, separate I4x4-derived right/bottom mode caches for CABAC I8x8 neighbour prediction, aligned-buffer reconstruction for partial edge macroblocks, FFmpeg-scale 8×8 dequant before IDCT, and correct I8x8 top-right references from already reconstructed rows. `GO264_RECON_TRACE=1` plus local FFmpeg instrumentation is the current tool path for isolating remaining prediction/residual/output mismatches. Raw CABAC 8×8 residual `levelseq` traces now match FFmpeg for the compared prefix, and `ff_raw_coeff` confirms Go raw coefficients convert exactly to FFmpeg `rawlevels` for the first residual-focused block. Enhanced FFmpeg `pred_block`/`res_block` traces showed Go's early 8×8 residual quadrants were 4× too large; the accepted dequant scale fix plus top-right reference fix lift `bbb` frame-0 luma to Y=42.56dB while leaving `testsrc_cabac_p` unchanged.
